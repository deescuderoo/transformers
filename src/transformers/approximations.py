...
def ref_softmax_store_in_file(x, layer_id, dim=None):
...
#this function is using the max value of the statistics across 3276 data points
    maxes = tensors[layer_id]
    maxes = maxes.to(x.device)
    x_exp = torch.exp(x-maxes)
    x_exp_sum = torch.sum(x_exp, dim, keepdim=True)
    return x_exp/x_exp_sum
